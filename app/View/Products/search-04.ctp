<style type="text/css">
    ...
    .in247s-top-ed__sitelinks,
    .in247s-bottom-ed__sitelinks {
        margin-bottom: 0;
    }
...
<div class="related-key-wrds">
    <div class="container">
        ...
        <?php
        echo $this->element(
            UI_SENTI_META_PRODUCTS,
            [
                "data" => [
                    "themeColors" => [
                        'primaryColor' => '#292929',
                        'secondaryColor' => '#1571ff',
                        'fontColor' => '##292929'
                    ],
                    "scoreLabel" => "247insider Score"
                ]
            ],
            array('plugin' => 'ArbPlugin')
        );
        ?>
    </div>
</div>
...
<div class="alignment-wrp">
    <div class="container">
        <div class="pull-left">
            <div class="listings">
                <?php if ($response->localFeedProductsList && !empty($response->localFeedProductsList->products)) { ?>
                    <?php echo $this->element('LocalListings', array("productsToDisplay" => $response->localFeedProductsList, "className" => 'lf-Serp-tgt')) ?>
                <?php } ?>
            </div>
            <?php if (Configure::read('device') == 'd') { ?>
                <div class="azure-img">
                    <?php
                    if ($response->showPrimaryImageMultimedia) {
                        echo $this->element('AzureImages', array("ImagesToDisplay" => $response->primaryImageMultimedia, "className" => 'az-im-Serp-di-tgt'));
                    }
                    ?>
                </div>
            <?php  } ?>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="419" height="79" viewBox="0 0 419 79">
                ...
                <g fill="none" fill-rule="evenodd">
                    ...
                    <text fill="#FFF" font-family="Ubuntu-Bold, Ubuntu" font-size="20" font-weight="bold" transform="translate(5)">
                        <tspan x="12" y="36">COMPARE</tspan>
                        <tspan x="12" y="56.24" font-family="Ubuntu-Regular, Ubuntu" font-size="16" font-weight="normal">&amp; SAVE UPTO</tspan>
                    </text>
                    <g fill="#FF8B66">
...
<?php if (Configure::read('device') == 'm') { ?>
    <div class="azure-img mob">
        <?php
        if ($response->showPrimaryImageMultimedia) {
            echo $this->element('AzureImages', array("ImagesToDisplay" => $response->primaryImageMultimedia, "className" => 'az-im-Serp-di-tgt'));
        }
