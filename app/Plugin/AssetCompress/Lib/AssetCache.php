...
class AssetCache {
    ...
    protected function _readTimestamp() {
        ...
        if (empty($data) && file_exists(ASSET_COMPRESS_TMP . AssetConfig::BUILD_TIME_FILE)) {
            $data = file_get_contents(ASSET_COMPRESS_TMP . AssetConfig::BUILD_TIME_FILE);
            if ($data) {
    ...
    protected function _writeTimestamp($data) {
        ...
        $data = serialize($data);
        file_put_contents(ASSET_COMPRESS_TMP . AssetConfig::BUILD_TIME_FILE, $data);
        chmod(ASSET_COMPRESS_TMP . AssetConfig::BUILD_TIME_FILE, 0644);
    }
