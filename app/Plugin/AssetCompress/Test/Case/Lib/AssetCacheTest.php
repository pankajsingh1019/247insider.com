...
class AssetCacheTest extends CakeTestCase {
    ...
    public function setUp() {
        ...
        $this->config->cachePath('js', ASSET_COMPRESS_TMP);
    ...
    public function testWrite() {
        ...
        $contents = file_get_contents(ASSET_COMPRESS_TMP . 'test.js');
        $this->assertEquals('Some content', $contents);
        unlink(ASSET_COMPRESS_TMP . 'test.js');
    }
    ...
    public function testWriteTimestamp() {
        ...
        $contents = file_get_contents(ASSET_COMPRESS_TMP . 'test.v' . $now . '.js');
        $this->assertEquals('Some content', $contents);
        unlink(ASSET_COMPRESS_TMP . 'test.v' . $now . '.js');
    }
    ...
    public function testIsFreshSuccess() {
        ...
        $this->config->set('js.timestamp', false);
        touch(ASSET_COMPRESS_TMP . '/libs.js');

        $this->assertTrue($this->cache->isFresh('libs.js'));
        unlink(ASSET_COMPRESS_TMP . '/libs.js');
    }
    ...
    public function testIsFreshConfigExpire() {
        ...
        $config->cachePath('js', ASSET_COMPRESS_TMP);
    ...
    public function testThemeFileSaving() {
        ...
        $this->config->cachePath('css', ASSET_COMPRESS_TMP);
        $this->cache = new AssetCache($this->config);

        $this->cache->write('themed.css', 'theme file.');
        $contents = file_get_contents(ASSET_COMPRESS_TMP . 'blue-themed.css');
    ...
    public function testTimestampFromCache() {
        ...
        unlink(ASSET_COMPRESS_TMP . AssetConfig::BUILD_TIME_FILE);
